    ...
    def jump(self):
        if not self.is_jumping:
            ...
            if keys[pygame.K_SPACE]:
                ...
                jump_sound.play()
                self.jump_time = 0  # Reiniciar el tiempo de salto
        else:
            self.jump_time += 1  # Aumentar el tiempo de salto
            if self.jump_time < 15:
                self.vertical_velocity += self.gravity
            else:
                self.vertical_velocity += self.gravity * 1.5  # Aumentar gravedad
...
def generate_obstacles(num):
    return [Obstacle(random.randint(0, WIDTH - width), random.randint(0, HEIGHT - height - 50), random.randint(50, 100), random.randint(20, 50)) for _ in range(num)]
